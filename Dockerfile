...
CMD ["sh", "-c", "dd=$(date +'%F_%T.%3N') && echo $dd && kubectl -n site exec --stdin --tty site-bitrix-app-mysql-0 -- /bin/bash -c  'mysqldump  -u root --password=iOPt6ZXGtn7zEjxsub3MoDLwAa51EQP8 --routines bitrix-app > /tmp/btrxApp_mysql_+$dd+.sql' && kubectl cp site/site-bitrix-app-mysql-0:/tmp/backup.sql /nfs-data/btrxApp_mysql_+$dd+.sql && ls -l /nfs-data"]
