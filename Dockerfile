...
CMD ["sh", "-c", "echo $(date +"%F_%T.%3N") && kubectl -n site exec --stdin --tty site-bitrix-app-mysql-0 -- /bin/bash -c  'mysqldump  -u root --password=iOPt6ZXGtn7zEjxsub3MoDLwAa51EQP8 --routines bitrix-app > /tmp/backup3.sql' && kubectl cp site/site-bitrix-app-mysql-0:/tmp/backup.sql /nfs-data/backup3.sql && ls -l /nfs-data"]
